...
The entries for GAP packages (those with `note = {GAP package}`) are generated
from the `PackageInfo.g` files of the packages shipped with the GAP.jl version
in the current environment. Do not edit them by hand; instead, in particular
after a GAP.jl update, run

    julia --project=. etc/update_gap_package_bib.jl

from the root directory of the Oscar.jl repository, followed by `bibtool` as
described above. The script also updates all `@cite` references whose keys
changed. Run it with `--check` to only report outdated entries.
